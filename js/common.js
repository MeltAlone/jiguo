 ...
 * 参数：
 *    {
 *      isBack:true,      // 是否回顶部功能，默认为true
 *      goBack:false,     // 是否返回上一页，默认为false。goback与isback同时为true时优先实现回顶部
 *      scrollTop:0,      // 滚动条高度多少开始出现
 *      position:"auto",  // position为位置信息，内容的左边或者右边
 *      width:1000,       // 距离网页内容模块的宽度
 *      offset:5,         // 距离网页内容模块间距
 *      bottom:"auto"     // 距离底部距离
 *    }
 */
	...
	$(".comMore").click(function(){
		...
		$.ajax({
			...
			for(var j=0;j<dlen;j++){
				...
				param+='<li><a href="#"><img src="'+img+'" width="220" height="130"/><div class="info"><p class="name">'+text+'</span></p><div class="tip fix"><span class="price left">'+price+'</span><div class="right icon"><span class="xin">3</span><span class="look">3</span></div></div></div></a></li>';
			}
			self.parent().prev().append(param);
			indexNum++;
			if(indexNum>=allLen){
...
$(window).on("hashchange", function(e){
	getHtmlContent(location.hash.substring(1));
	function getHtmlContent(locHash) {
		$.ajax({
			type: "get",
			url: `${locHash}`,
